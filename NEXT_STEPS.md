...
- ✅ `_collect_default_visuals`, state-channel baselines, resumable history, and `ServerLayerMirror` hydrate from `SceneBlockSnapshot.layers` (via `scene_layers.*.block`) and emit LayerBlock deltas.
- ✅ Shared LayerBlock diff helpers power both runtime reapply and the notify path, so everything downstream of the ledger reads the new blocks (we still emit the legacy wire schema until the protocol flip).
- ⚠️ Notify payload schema remains legacy (`LayerVisualState`); we must flip the protocol and drop the shim before deleting `RenderLedgerSnapshot.layer_values`.

## Immediate Next Steps (Phase 3 Cleanup)
1. **Protocol flip (no compat clients)**
...
| ---- | ----- | --------- | ------------- |
| **Pass A (DONE)** | Control/notify block ingestion | - Baselines, mirrors, and notify/history emission ingest LayerBlocks; shared diff helper wired everywhere | Server-side state/notify code no longer reads `RenderLedgerSnapshot.layer_values`. |
