...
- Reuse existing headless primitives (`ViewerSceneManager`, `ServerSceneState`, `ServerSceneQueue`) instead of instantiating Qt objects.
...
- **Scene state**: the authoritative scene lives in `ServerSceneState` plus the worker viewer model. MCP commands should apply mutations through the same queues (`ServerSceneQueue` and camera command deque) that the WebSocket state channel uses.
