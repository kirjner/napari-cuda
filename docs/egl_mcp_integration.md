...
- Reuse existing headless primitives (`ViewerSceneManager`, `ServerSceneState`, `SceneStateQueue`) instead of instantiating Qt objects.
...
- **Scene state**: the authoritative scene lives in `ServerSceneState` plus the worker viewer model. MCP commands should apply mutations through the same queues (`SceneStateQueue` and camera command deque) that the WebSocket state channel uses.
...
2. **Headless tool bindings**
   - Implement tool methods using existing server helpers:
     ...
     - Mutations: enqueue updates via `_latest_state`, `_enqueue_camera_command`, and the worker's request queues.
