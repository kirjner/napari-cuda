...
| `notify.dims` | `type`, `version`, `session`, `timestamp`; omit `seq`/`delta_token` | `current_step`; `ndisplay`; `mode`; `source`; optional `intent_id` | `current_step`, axis labels, and multiscale shapes are emitted in viewer/display order; clients MUST consume them as-is (no permutations). `ndisplay` mirrors the snapshot’s viewer block; local intents carry `intent_id`, otherwise emit an 8-char `frame_id`. |
