...
- `worker.viewport_tick.run` asks the `ViewportRunner` for a `ViewportPlan`. If the plan contains a slice task and we are still in plane mode, it routes through `snapshots.plane.apply_slice_roi` (`src/napari_cuda/server/runtime/worker/viewport_tick.py`).
...
4. Update worker helpers (`_ensure_scene_source`, `worker.viewport_tick.run`, `_build_scene_state_context`) to read/write through the new struct, guarding mutations with `_state_lock` where required.
