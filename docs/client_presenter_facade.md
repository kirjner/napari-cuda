...
- A dedicated façade gives `NapariLayerMirror` and friends a stable hook while keeping the Qt canvas focused on window deferral and keymap setup.
...
- `NapariLayerMirror` registers here so Qt layer widgets can emit through the façade without poking at the loop or canvas.
