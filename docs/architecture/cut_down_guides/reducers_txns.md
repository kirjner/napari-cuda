...
- `reduce_dims_update(axis, prop, value|step_delta)`
  - Index/step: clamp against `level_shapes[current_level][axis]`; write `dims_spec` and `current_step`.
...
- `reduce_level_update(level, step?)`
  - Set `current_level`; optionally remap/clamp `current_step` and update `level_shapes` if shape provided.
...
- `apply_level_switch_transaction(ledger, op_seq, level, step?, dims_spec_payload, origin, ts)`
