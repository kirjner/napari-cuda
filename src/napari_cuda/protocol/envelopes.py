"""Helper builders for the spec-compliant control protocol envelopes.

This module wraps the dataclasses defined in :mod:`napari_cuda.protocol.messages`
with convenience constructors that enforce the envelope contract captured in
``docs/protocol_greenfield.md`` (see Appendix B).

It deliberately re-exports every frame/payload class so imports that target
``napari_cuda.protocol.envelopes`` stay stable while we finish migrating call
sites to the new builders.
"""
