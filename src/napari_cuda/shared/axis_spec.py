"""Axis metadata consolidation helpers."""
...
class AxesSpec:
    ...
    plane_mode: bool
    axes: tuple[AxesSpecAxis, ...]

    def axis_by_label(self, label: str) -> AxesSpecAxis:
        target = str(label)
        for axis in self.axes:
            if axis.label == target:
                return axis
        raise KeyError(f"axes spec missing label={target}")

    def axis_by_index(self, index: int) -> AxesSpecAxis:
        target = int(index)
        for axis in self.axes:
            if axis.index == target:
                return axis
        raise KeyError(f"axes spec missing index={target}")


LedgerSnapshot = Mapping[tuple[str, str, str], Any]


def build_axes_spec_from_ledger(snapshot: LedgerSnapshot) -> AxesSpec:
...
def axes_spec_to_payload(spec: AxesSpec | None) -> dict[str, Any] | None:
    if spec is None:
        return None
    axes_payload = []
...
def axes_spec_from_payload(data: Mapping[str, Any] | None) -> AxesSpec | None:
    if data is None:
        return None
    axes_payload = []
    axes_seq = data.get("axes")
    assert isinstance(axes_seq, Sequence), "axes spec payload requires axes sequence"
    for entry in axes_seq:
        assert isinstance(entry, Mapping), "axes spec axis payload must be mapping"
...
def axes_spec_from_notify_payload(payload: Any) -> AxesSpec | None:
    data = getattr(payload, "axes_spec", None)
    if isinstance(data, Mapping):
        return axes_spec_from_payload(data)
    if data is None:
        return None
    raise TypeError("notify.dims axes_spec payload must be mapping or None")


def validate_ledger_against_spec(spec: AxesSpec, snapshot: LedgerSnapshot) -> None:
    current_step_raw = _require_value(snapshot, "dims", "main", "current_step")
    level_shapes_raw = _require_value(snapshot, "multiscale", "main", "level_shapes")
    current_level_raw = _require_value(snapshot, "multiscale", "main", "level")
    ndisplay_raw = _require_value(snapshot, "view", "main", "ndisplay")
