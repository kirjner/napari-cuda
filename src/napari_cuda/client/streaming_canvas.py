...
class StreamingCanvas(VispyCanvas):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        fps_guess = env_float(
            'NAPARI_CUDA_CLIENT_DISPLAY_FPS',
            env_float('NAPARI_CUDA_SMOKE_FPS', 60.0),
        )
        ...
        if self._use_manager:
            ...
            use_disp_loop = (
                (_os.getenv('NAPARI_CUDA_USE_DISPLAY_LOOP', '0') or '0')
            ).lower() in ('1', 'true', 'yes', 'on')
            if use_disp_loop:
                from napari_cuda.client.streaming.display_loop import (
                    DisplayLoop as _DisplayLoop,
                )

                cfg = getattr(self, '_client_cfg', None)
                fps_val = float(cfg.draw_fps) if cfg is not None else None
                self._display_loop = _DisplayLoop(
                    scene_canvas=self._scene_canvas,
                    fps=fps_val,
                    prefer_vispy=True,
                )
                ...
                logger.info(
                    "DisplayLoop enabled (vispy timer preferred) for steady 60 Hz cadence"
                )
        else:
            pass

        # Override draw to show video instead, but keep play-enable hook intact
        draw_events = self._scene_canvas.events.draw
        with contextlib.suppress(ValueError, RuntimeError):
            draw_events.disconnect(self.on_draw)
        # Ensure dims play continues to tick
        draw_events.connect(self.enable_dims_play, position='first')
        ...
        view_hud_env = (
            (os.getenv('NAPARI_CUDA_VIEW_HUD') or '0').lower()
            in ('1', 'true', 'yes', 'on')
        )
        metrics_env = (
            (os.getenv('NAPARI_CUDA_CLIENT_METRICS', '0') or '0').lower()
            in ('1', 'true', 'yes', 'on')
        )
        # Enable the overlay when FPS HUD requested, metrics enabled, smoke mode, or view HUD requested
        self._hud_enabled = (
            (hud_env == '1')
            or bool(self._vt_smoke)
            or bool(metrics_env)
            or bool(view_hud_env)
        )
    ...
    def _init_vt_from_avcc(
        ...
        try:
            ...
            logger.exception('VT live init failed: invalid AVCC payload', exc_info=True)
            return
    ...
    def _update_fps_hud(self) -> None:
        ...
        vt_q_len = None
        vt_pipeline = getattr(mgr, '_vt_pipeline', None)
        if vt_pipeline is not None and hasattr(vt_pipeline, 'counts'):
            vt_counts = vt_pipeline.counts()
        ...
        pyav_pipeline = getattr(mgr, '_pyav_pipeline', None)
        q_py = 0
        ...
        if (os.getenv('NAPARI_CUDA_HUD_LOOP_SHOW_MEAN') or '0').lower() in (
            '1',
            'true',
            'yes',
        ):
        ...
        if (os.getenv('NAPARI_CUDA_HUD_SHOW_MEM') or '0').lower() in (
            '1',
            'true',
            'yes',
        ):
            ...
            if _ps is not None:
                proc = _ps.Process()
                cur_mb = float(proc.memory_info().rss) / (1024.0 * 1024.0)
            try:
        ...
        show_jit = (jit_q is not None) or (
            jit_deliv_rate is not None
        ) or (jit_sched_mean is not None)
        if show_jit:
            parts = []
